...
public class TelegramBotService
{
    ...
    public TelegramBotService(string botToken, JiraClient jiraClient, string botUsername, MediaHandlerService mediaHandlerService, AppDbContext context, IChatConfigService chatConfigService, Config config, string channelId, NotificationService notificationService, SendDashboardService sendDashboardService)
    {
        ...
        _sendDashboardService = new SendDashboardService(_botClient);
...
private async Task<bool> ProcessCommandAsync(Message message)
{
    ...
    if (message.Text.StartsWith("/dashboard"))
    {
        ...
        await _sendDashboardService.SendDashboardScreenshotAsync(message.Chat.Id, dashboardUrl, caption, loginUrl, username, password);
        return true;
    }
